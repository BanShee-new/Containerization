...
Для реализации задачи создаем манифест файл с названием compose.yaml :

>vim compose.yaml

Наполняем созданный файл содержимым, соблюдая отступы, где:  

* первый сервис с названием db скачает образ mariadb с docker hub и создаст контейнер с этим образом, этот сервис будет всегда перезапускать контейнер, если он остановлен и имеет одну переменную с паролем к базе данных.

* второй сервис с названием adminer скачает образ adminer с docker hub и создаст контейнер с этим образом, сервис будет всегда перезапускать этот контейнер если он остановлен и откроет порт 6080 для связи с первым контейнером
...
После скачивания образов произайдет запуск контейнеров.  
...
Проверяем через curl, доступен ли наш adminer:
> curl http://localhost:6080/

Просматриваем логи сервисов:
